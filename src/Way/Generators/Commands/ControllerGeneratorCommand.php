...
class ResourceGeneratorCommand extends Command {
    ...
    public function fire()
    {
        ...
        // All done!
        $this->info(sprintf(
            "All done! Bellow are routes which you need to add to routes.php :) 

\n// {$this->getTableName($resource)} create, save, show, edit, update, destroy
\n Route::group(array('prefix' => '{$this->getTableName($resource)}'), function()
\n{
Route::get('/', array('as' => '{$this->getModelName($resource)}Index', 'uses' => '{$this->getControllerName($resource)}@index'));
\n    Route::get('create', array('as' => '{$this->getModelName($resource)}Create', 'uses' => '{$this->getControllerName($resource)}@create'));
\n    Route::post('store', array('as' => '{$this->getModelName($resource)}Store', 'uses' => '{$this->getControllerName($resource)}@store'));
\n    Route::get('edit/{id}', array('as' => '{$this->getModelName($resource)}Edit', 'uses' => '{$this->getControllerName($resource)}@edit'));
\n    Route::get('show/{id}', array('as' => '{$this->getModelName($resource)}Edit', 'uses' => '{$this->getControllerName($resource)}@show'));
\n    Route::post('update/{id}', array('as' => '{$this->getModelName($resource)}Update', 'uses' => '{$this->getControllerName($resource)}@update'));
\n    Route::get('destroy/{id}', array('as' => '{$this->getModelName($resource)}Destroy', 'uses' => '{$this->getControllerName($resource)}@destroy'));
});\n\n" 
        ));
